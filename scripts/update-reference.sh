...
echo "==> Building tests..."
cmake --build "$REPO_ROOT/build" --target Tests -j"$(sysctl -n hw.logicalcpu 2>/dev/null || nproc)"

echo "==> Running snapshot tests to generate new references..."
"$REPO_ROOT/build/Tests/Tests" --gtest_filter="AudioRenderingTest.Snapshot*"
