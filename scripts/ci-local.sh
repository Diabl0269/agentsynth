...
#      just its unit test).
#   5. scripts/check-docs.sh against the real tree -- docs link/anchor/naming integrity: markdown
...
#   6. Every scripts/tests/*.test.sh -- ci-cache-check, ci-install-linux-deps,
#      check-nonascii-literals, ai-eval-ratchet, utf8-literal-check, check-file-sizes,
#      check-function-sizes, check-docs as of this writing, globbed so a newly added one is
#      picked up automatically. check-nonascii-literals.test.sh's own last case scans the real
#      Source/ tree, so this also covers the Lint job's ASCII-literal gate.
#   7. Configure (-DENABLE_TESTS=ON -DENABLE_AI_HARNESS=ON, Release, matching the macOS/Windows
...
#   8. Dev-sign the built app bundle with scripts/dev-sign-app.sh (macOS only -- not a CI check,
#      but the point where a local build exists to sign; see that script's header for why).
#   9. Run the full suite: build-ci-local/Tests/Tests.
#
...
#   --skip-tests  Skip step 9 (the full Tests suite, often the slowest step). Everything else
...
file-size guard, the function-size guard, the docs integrity guard, every
scripts/tests/*.test.sh, then a full Release build of every CMake target CI
builds (Core, AppUI, AgentSynth, AgentSynthPlugin, Tests) with
...
full mapping to what CI actually runs.
  ...
  --skip-tests  Skip the Tests suite (step 9), often the slowest step.
...
# --- 5. Docs guard, against the real tree -------------------------------------------------------
...
# --- 7. Configure + build every target the CI build-and-test jobs build -----------------------
...
# --- 8. Dev-sign the app bundle (macOS only, before tests so a signing failure surfaces early) --
