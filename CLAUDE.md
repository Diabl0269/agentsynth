...
- **A node's uuid must be mirrored into its processor at every write site** — `node->properties.set("uuid", ...)` is message-thread-only data the audio thread can't read, so all three writers in `AIStateMapper.cpp` pair it with `mirrorUuidIntoProcessor` (→ `ModuleBase::setNodeUuid`). Add a fourth writer without the mirror and a Track In node silently stops matching its timeline track. The lock-free `getNodeUuid()` read is only sound because the uuid transitions empty→value exactly once, before the node is audio-visible — never rewrite or clear it. → [`docs/architecture.md`](docs/architecture.md)
