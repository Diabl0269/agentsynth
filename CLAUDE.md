...
- **Timeline data crosses threads only via the snapshot exchange** — the audio thread reads the timeline through `TimelineSnapshotExchange::beginAudioBlock()`, once per callback; never cache the returned reference across blocks (epoch reclamation frees a superseded snapshot two callbacks after retirement, so a cached pointer is a use-after-free). Modules don't call it: `AudioEngine::renderNextBlock` parks the block's snapshot on the transport, and a module downcasts `getPlayHead()` to `synth::TransportService*` to read it — same block-only lifetime, plus it can be null. On the message side, every `TimelineDoc` edit goes through its mutation API — writing `tracks` directly silently breaks the revision counter, listeners, snapshot republication and (later) undo. → [`docs/architecture.md`](docs/architecture.md)
