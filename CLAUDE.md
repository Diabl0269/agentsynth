...
~400 tests across ~56 suites, all headless (no audio device, no GUI window). Seven test layers: audio rendering (DSP verification), integration (signal chains, mod routing), component workflow (UI interactions), state management (presets, undo/redo, serialization), theme system (token round-trips, ColourId mapping, WCAG contrast), layout (grid snap, anti-overlap, auto-arrange), and E2E workflow (full application paths). Code coverage threshold: 85%. See [`docs/testing.md`](docs/testing.md) for the full breakdown, patterns, and how to add tests for new modules.
...
- `Tests/`: ~400 tests across ~56 suites (audio rendering, integration, component workflow, state management, theme, layout, E2E workflow)
...
- `docs/layout.md`: Soft-grid model, kGridSize=8 rationale, anti-overlap spiral search, auto-arrange topological layering algorithm (spacing constants, role ranks, undo integration), and LayoutUtil API reference
