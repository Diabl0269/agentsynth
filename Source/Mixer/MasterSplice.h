...
/**
 * Returns the graph's Master node, splicing one in first if there is none (docs/mixer.md §5.1).
 *
 * Follows MainComponent::ensureMasterRecordTap()'s pattern one level upstream of it:
 ...
 *   - node, uuid, position and the whole re-splice are ONE compound undo step
