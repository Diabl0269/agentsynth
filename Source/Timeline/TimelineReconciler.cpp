...
bool TimelineReconciler::reconcile(TimelineDoc& doc, const juce::AudioProcessorGraph& graph) {
    // uuid -> processor, built once: the per-lane check needs the processor (to test whether the
    ...
    // A lane's node existing is not the whole story once that node is a HostedPluginModule —
    ...
    // exactly as it was before this check existed — a lane whose paramId has gone missing on one of OUR OWN
