...
// docs/midi_remote.md §7's "Core never touches juce::ApplicationProperties" line). This store is
// injected into whatever app-layer owner needs it (MainComponent) and lives under
 ...
 *        <profileId>.json` (docs/midi_remote.md §7).
 *
...
public:
    ...
    /** Plain file copy of `<profileId>.json` to `destFile` (docs/midi_remote.md §7: "Export ... is
     *  a file copy"). Fails if the profile isn't in this store. */
     ...
     *
     *  Design decision (docs/midi_remote.md §7 says only "a name check", underspecified beyond
     *  that): refuses — returns false, copies nothing — if a profile with that id ALREADY EXISTS
