...
// --- Math ---
// Math has 2 inputs (A, B on ch0/ch1) aliased with 5 outputs (Sum, Diff, Min, Max, Mult on
// ch0..ch4). Bypass passes ch0 (input A) through untouched and clears every other channel
// (including ch1, input B) so nothing derived leaks downstream.

TEST(FXBypassTest, MathBypassPassesDryAudio) {
    MathModule module;
    module.prepareToPlay(44100.0, 512);
    ...
    buffer.clear();
    for (int i = 0; i < 512; ++i) {
        buffer.setSample(0, i, 0.9f); // A
        buffer.setSample(1, i, 0.4f); // B
    }
    ...
    for (int i = 0; i < 512; ++i) {
        EXPECT_FLOAT_EQ(buffer.getSample(0, i), 0.9f) << "Ch0 (input A) sample " << i;
    }
}

TEST(FXBypassTest, MathBypassClearsDerivedOutputs) {
    MathModule module;
    module.prepareToPlay(44100.0, 512);
    ...
    for (int i = 0; i < 512; ++i) {
        buffer.setSample(0, i, 0.9f); // A
        buffer.setSample(1, i, 0.4f); // B (non-zero — must not leak through as Diff)
        buffer.setSample(2, i, 0.3f);
        buffer.setSample(3, i, 0.3f);
        buffer.setSample(4, i, 0.3f);
    }
...
TEST(FXBypassTest, MathMuteSilencesOutput) {
    MathModule module;
    module.prepareToPlay(44100.0, 512);
