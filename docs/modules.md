...
- **Splitting a module that was wired while collapsed brings both legs up live** (`GraphEditor::completeStereoPairConnections`), and the two sides are deliberately *not* symmetric — see [`fx_modules.md § Splitting a split-block module that was wired mono`](fx_modules.md#splitting-a-split-block-module-that-was-wired-mono) for the full rule and the one leg it cannot connect.
