...
- **Binding is by node uuid**, and by **kind**: it finds "its" track by scanning the snapshot for the first **Audio** track whose `bindingUuid` `strcmp`s equal to `ModuleBase::getNodeUuid()`. An empty uuid matches nothing. (This is why the binding chip's menu is kind-aware — see [timeline_panel_core.md §3](timeline_panel_core.md).)
