...
- **Bypass/mute contract is unchanged.** `applyOutputLevel` is never reached on the bypass branch (dry pass-through stays untouched, so Level cannot silence a bypassed module) nor on mute (already cleared). See [`architecture/module-base.md#bypassmute-contract`](../architecture/module-base.md#bypassmute-contract).
...
- **Channels**: 15 in / 2 out — `0-1` stereo audio, `2` B2 Freq CV, `3` B2 Gain CV, `4` B3 Freq CV, `5` B3 Gain CV, `6` B1 Freq CV, `7` B1 Gain CV, `8` B4 Freq CV, `9` B4 Gain CV, `10` B1 Q CV, `11` B2 Q CV, `12` B3 Q CV, `13` B4 Q CV, `14` Output CV. Every continuous parameter is a target (`docs/modules/modulation.md#every-continuous-parameter-is-a-target`); channels 6-14 were appended after the original 2-5 so a saved patch that modulates B2/B3 keeps working.
- **CV mapping**: the two bells' Freq/Gain (ch2-5) predate the rule and keep their bespoke curves — Freq CV is exponential over the full 20 Hz – 20 kHz range (`+1` sweeps to 20 kHz, `-1` to 20 Hz, matching FilterModule's cutoff-CV feel), Gain CV maps linearly onto the full ±24 dB range and adds to the knob value. Every jack added under the rule (ch6-14: B1/B4 Freq+Gain, every band's Q, Output) instead reads the normalised convention — `ModuleBase::blockCV` + `modulateNormalised`, base + CV in the parameter's own 0-1 range (`docs/modules/modulation.md#cv-in-normalised-units`). All CV is sampled once per block; the bells' custom read is RMS-gated so an unpatched jack reads as exactly 0, the normalised jacks gate the same way via `blockCV` reading 0 when nothing is connected.
