...
| `ADSRModule` (mono MIDI branch) | Sample-accurate | Its per-sample loop checks each event's `samplePosition` against the current sample and calls `noteOn()`/`noteOff()` at the exact sample it happens — a note-on always fires `noteOn()` immediately (FRO110), not only on an edge of the held/not-held state, which is what lets a note-off + note-on landing on the same sample still re-articulate. |
| `SamplerModule` | Sample-accurate (fixed) | `applyHeldNoteMessage` is called once per message at its own `samplePosition` inside the per-sample render loop, updating a `heldNotes` bitset; a Note-On always retriggers immediately (FRO246, mirrors ADSRModule's FRO110), not only on an edge of `heldNotes.any()`. Previously latched a block-net `midiGateOpen`/`heldMidiNote` pair before the render loop ran, which silently dropped a legato Note-On (no Note-Off between them) or the Note-On half of a same-block Note-Off+Note-On pair (e.g. a transport loop restart landing inside one block). |
