...
A freshly created `git worktree add` checkout starts with no `build-ci-local/` at all, so its first configure has nothing in `build-ci-local/_deps` and FetchContent re-downloads JUCE (~600 MB), GoogleTest and Sparkle from scratch — 15-20 minutes before a single line of C++ compiles, even though the main checkout sitting right next to it (under `agentsynth-suite/`) already has those exact sources on disk. Found 2026-09-15 running two worktrees in parallel for the mixer epic, where that repeated every time.
