...
| ADSRTest | 21 | Attack/sustain/release shapes, retriggering, poly mode, parameter changes during playback, **mono Gate CV** (rising edge starts, falling edge releases, MIDI-only still works, MIDI+Gate OR so note-off does not release while CV is high), **Threshold** (default 0.5, raised threshold rejects a sub-threshold gate, Threshold CV, hysteresis), port layout (Gate + Threshold) |
...
| FXBypassTest | 29 | Per-FX bypass dry pass-through, CV-channel clearing, mute silencing |
...
| PreferencesSettingsTabTests | 5 | Preferences tab defaults (`NewAndUnwired`, double-click disconnect on), load/save of `smartConnectionMode` and `doubleClickPortDisconnect`, push to `GraphEditor` via `setGraphEditor`, paint/resize smoke |
