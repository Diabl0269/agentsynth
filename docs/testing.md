...
> **Do not rename the `Lint`, `Build, Test, and Coverage`, `Build and Test (macOS)`, `Build and Test (Windows)`, `Docs`, or `PR Title` jobs.** Those six strings are configured as required status checks in `main`'s branch protection (FRO170 added the last two); renaming one leaves a required check permanently pending and blocks every merge. Skipping a job on `push` is safe (protection only gates pull requests) — renaming it is not. On a docs-only PR, this workflow's own four jobs don't run at all (see [Docs-only PR coverage](#file-size-cap-lint-job) below) — `.github/workflows/ci-passthrough.yml` is what keeps them from blocking merge in that case.
...
**Docs-only PRs are gated by this too, just via a different job.** `ci.yml`'s `paths:` filter (see [CI Pipeline](#ci-pipeline) above) deliberately does not include `docs/**` or `*.md` — adding them would trigger the full build matrix for a docs-only change, which is what the filter exists to avoid — so `ci.yml`'s own Lint job never runs for one. `.github/workflows/docs.yml`'s "Docs" job (required as of FRO170 — see [`docs/docs-guard.md`](docs-guard.md#enforcement-fro170)) runs this same check unconditionally on every PR, so a docs-only PR is gated in CI, not only by the pre-commit hook and `scripts/ci-local.sh` (which still run the guard unconditionally against the whole tree, catching it before it's ever pushed). A mixed code+docs PR is gated by both — `Source/**` / `scripts/**` / etc. changing triggers the Lint job the guard also rides in.
