...
| EnvelopeGeneratorTest | 12 | `Tests/Modules/Envelope/EnvelopeGeneratorTests.cpp` — the shared `synth::EnvelopeGenerator` engine ADSRModule is built on, independent of the module wrapper: zero-duration-stage cascade (a 0 ms stage costs no samples), exact stage timing across a curve sweep, `shape()` endpoint/monotonicity, release from mid-attack/mid-decay starting at the current level, retrigger-from-current-level (never 0, never instant), a mid-ramp time change with no level discontinuity, sustain 0/1 edge cases, and a gapless note sequence at sustain 0 |
