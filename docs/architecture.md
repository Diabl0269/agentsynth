...
- **Applier semantics.** `applyBlock(table, blockInfo, recordState)` is `noexcept`, allocation-free, lock-free and log-free. Three deliberate rules: **not playing ⇒ nothing is written** (a stopped transport leaves every knob free for the user); **`setValue`, never `setValueNotifyingHost`** (a plain store — notifying would fire parameter listeners from the audio thread for every automated parameter on every block; UI reflection of automation is TL4-5, on a message-thread timer, and since TL4-4 notifying here would also feed playback straight back into the recorder's own parameter listener, so every pass would re-record itself); and values **clamp into the parameter's current range** via `RangedAudioParameter::convertTo0to1`, which clamps to [0, 1] itself, so a lane authored against a wider range pins at the endpoint. A non-finite value is skipped rather than handed to JUCE (`NormalisableRange` `jassert`s on NaN). Each binding owns its `AutomationCursor`, so swapping tables swaps the cursors with them and there is nothing to reset. On top of all that, TL4-4's **per-lane record mode** decides whether the lane may write this block at all — see the record-modes table below.
