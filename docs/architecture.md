...
module follows, plus an index into the topic docs that carry the detail (split out under
FRO163 — this file used to hold all of it and sat at the 1000-line cap, which meant no
change could land here without going over).
...
The detail for each numbered component below now lives in its own doc — the section numbers
are kept stable here (and in every `architecture.md §N` reference elsewhere in the repo) even
though the content moved:

1. [AudioEngine](architecture_audio_engine.md#1-audioengine) — the realtime render graph, host modes, device callback
2. [TransportService (the one clock)](architecture_audio_engine.md#2-transportservice-the-one-clock) — bounce/export, stem export, metronome, input monitoring, mixer solo gate
3. [TimelineDoc (the timeline document model)](architecture_timeline.md#3-timelinedoc-the-timeline-document-model)
4. [TimelineSnapshot (the audio thread's view of the timeline)](architecture_timeline.md#4-timelinesnapshot-the-audio-threads-view-of-the-timeline) — AutomationKernel, AutomationApplier, AutomationRecorder, UI reflection
5. [ProjectBundle (.agsproj)](architecture_project_bundle.md#5-projectbundle-agsproj) — open/save, recent projects, dirty state, autosave, welcome screen
6. [ModuleBase](architecture_module_base.md#6-modulebase) — ModuleType enum, node uuid mirror, ModulationTarget, VisualBuffer, logical-port API, extra state, bypass/mute contract, output level stage
7. [GraphEditor](architecture_graph_editor.md#7-grapheditor) — the visual patching canvas and its collaborator classes
8. [App wiring — who owns the timeline, and every hook that keeps it in step](architecture_app_wiring.md#8-app-wiring--who-owns-the-timeline-and-every-hook-that-keeps-it-in-step) — focus regions, audio recording, latency alignment, device changes, waveform peaks

Two components that don't carry a number, also under App wiring's doc:

- [AudioClipStreamer (disk-streaming clip playback)](architecture_app_wiring.md#audioclipstreamer-disk-streaming-clip-playback)
- [Asset management](architecture_app_wiring.md#asset-management)

## Plugin Layer

See [`architecture_plugin_layer.md`](architecture_plugin_layer.md) — host modes, ownership,
...
See [`architecture_module_base.md`](architecture_module_base.md#supporting-components) —
