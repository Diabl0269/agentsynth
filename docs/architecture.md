...
- **Applier semantics.** `applyBlock(table, blockInfo)` is `noexcept`, allocation-free, lock-free and log-free. Three deliberate rules: **not playing ⇒ nothing is written** (a stopped transport leaves every knob free for the user; TL4-4's latch/touch record modes are what refine this); **`setValue`, never `setValueNotifyingHost`** (a plain store — notifying would fire parameter listeners from the audio thread for every automated parameter on every block; UI reflection of automation is TL4-5, on a message-thread timer); and values **clamp into the parameter's current range** via `RangedAudioParameter::convertTo0to1`, which clamps to [0, 1] itself, so a lane authored against a wider range pins at the endpoint. A non-finite value is skipped rather than handed to JUCE (`NormalisableRange` `jassert`s on NaN). Each binding owns its `AutomationCursor`, so swapping tables swaps the cursors with them and there is nothing to reset.
...
- **Testing.** `Tests/AutomationApplierTests.cpp` (binding resolution, the playing-only rule, range clamping, deleted-node safety, republish-under-render) and `Tests/AutomationSlicingTests.cpp` (the four facts above). Both drive a `HostMode::Hosted` engine through `synth::OfflineTransportDriver` and assert on parameter values or rendered audio, never on applier internals.
