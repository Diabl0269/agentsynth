...
|---|---|---|
| `JuceModules` | Static library | The one shared compiled copy of the JUCE modules (`cmake/JuceModules.cmake`). `Core` links it PUBLIC, which hands every consumer the modules' headers and definitions without compiling them again; `Tests` and the `Tools/` harnesses take their JUCE code from it. `AgentSynth` and `AgentSynthPlugin` still compile their own copy, because JUCE's module code depends on their per-target definitions (`JUCE_STANDALONE_APPLICATION`, `JucePlugin_*`). |
