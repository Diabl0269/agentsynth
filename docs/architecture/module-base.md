...
Everything from `AudioInput` on is an internal-only or singleton node: `AudioInput` (a patch singleton), and `TimelineMidiSource` / `RecordTap` / `TimelineAudioSource` / `HostedPlugin` / the four Macro port types / `ChannelStrip` / `Master` (see [`docs/mixer/mixer.md`](../mixer/mixer.md)), which the module library never offers and a model may never author.
