...
- `Source/UI/Layout/LayoutUtil.h/.cpp`: Stateless grid-layout helpers (`snap`, `intersectsAny`, `findFreeSlot`, `computeAutoArrange`); no JUCE GUI deps — all geometry in canvas coordinates; Phase 3 adds width-bucket constants (`kNarrowWidth=40`, `kSingleWidth=280`, `kDoubleWidth=560`) and `getModuleWidthBucket(ModuleType)` / `moduleWidth()` helpers (Sequencer/PolySequencer/MidiKeyboard→Double, Attenuverter→Narrow, all others→Single); `kColumnStride = kSingleWidth + kLayerGapX = 360`; see `docs/layout.md` for the full model and API reference
